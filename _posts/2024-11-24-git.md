...
3. Dosya takibi
    - Repodaki dosya üç durumda bulunabilir.
      - [X] Modified: Değiştirilmiş fakat ***katkılanmamış(committed)*** dosya.
      - [X] Staged: Dosya katkılanmak üzere işaretlenmiş. 
      - [X] Committed: Dosya katkılanmış.
...
## Branch
Projemizi farklı dallara ayırarak ***master dalını(branch)*** istenmeyen değişikliklerden koruyabiliriz.
...
>Eğer uzak sunucudaki dosyaları değiştirmişsek `git pull` komutu ile onları yerel repoya çekebiliriz.
{: .prompt-info }
...
### Commit
Commit aşamasında dosyaya katkının eklenebilmesi için isim veya email den en az birini girmemiz gerekiyor. 
...
Yukarıda `git branch` komutu ile *test-branch* dalını oluşturup değişiklikler yapmıştık. Dilersek master dalıyla tekrar birleştirebiliriz. Bunun için önce master dalına geçiş yapıp sonra *test-branch* ile birleştiriyoruz.
...
**ksshashpass** oturum açmanızı isteyecek ve oturum bilgilerinizi **KWallet**'e kaydedecek. GitHub kullanıcı adı ve ardından da access token'ı girdikten sonra değişikliklerimiz uzak sunucuya yüklenecektir.
