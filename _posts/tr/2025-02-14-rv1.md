...
Bilgisayarlar önceden belirlenmiş komutları yürüten makinelerdir. Bu yürütmenin gerçekleşebilmesi için komutun bellekten okunması gerekir. Program Sayacı bu işlemi kontrol eden önemli parçalardan biridir.
...
Aşağıdaki diyagramda Program Sayacı, **Komut Hafızasında(Instruction Memory)** *0x0008 adresine* işaret ediyor ve Komut Hafızası da bu adresteki *0xDEDE komutunu* yürütülmesi için çıkışa yazıyor. 
...
Giriş çıkış ve sinyalleri **_i, _o, _s** biçiminde son ekler eklemek, karmaşık projeler ve *testbenchler* ile çalışırken kolaylık sağlayacaktır.
...
module pc(
    ...
    always @(posedge clk_i) begin
        //Eğer reset sinyali aşağıya çekilmişse çıkış 0
        if (~rstn_i) begin
            inst_o <= 0;
        end
...
![Desktop View](/assets/img/pc_tb.svg){: .normal}
...
module test (
  ...
  // Simülasyon başlangıç durumunu belirle
  initial begin
      clk = 0;
      rstn = 1;
...
Simülasyona için önce *pc_tb.v ve pc.v* kaynak dosyalarını derlememiz gerekiyor. Bunun için **iverilog** programını kullanacağız.
...
*Zamanlama Diyagramları ile ilgili yazıma [**buradan**](/posts/tr/wave)
ulaşabilirsiniz.*
...
GtkWave yukarıdakine benzer bir grafik gösterecektir. Fakat yukarıdaki diyagramı daha detaylı açıklayacak olursak; **addr_o** sinyalindeki taralı alan başlangıçtaki **belirsizlik(x)** durumunu ifade ediyor. Dijital devrelerdeki belirsizlik birçok istenmeyen duruma sebep olabilir. Bu yüzden hemen her dijital sistemde bir **reset devresi** bulunur ve sistem başlatıldığında bir **reset rutini** uygulanır.
...
Yazının son bölümünde davranışınu modellediğimiz bir devrenin gerçekte nasıl görünebileceğine bakalım. 

Bunun için **yosys** adlı sentez aracını kullanacağız.

`yosys -p "prep -top pc -flatten; write_json pc_top.json" pc.v`

Yosys yazdığımız Verilog kodunun *davranışını* analiz etti ve üzerinde bazı optimizasyonları yaparak bir şematiğe dönüştürdü.  Fakat bu şematiğin daha anlaşılır görünmesi için **netlistsvg** aracını kullandım. 

`netlistsvg pc_top.json`



![Behavioural Schematic](/assets/img/pc.svg){: width="1000" height="1000"}




Gördüğümüz üzere reset durumunun koşullu olması, multiplexer tarafından kontrol edilen bir devreye çevirildi. Girişlere 0 ve toplayıcı devresi bağlandı. Sonuç 32bitlik bir kaydedicide tutuluyor ve *clk_i* sinyaline bağlı olarak *addr_o* çıkışına yazılıyor. Bu çıkış ileride komut hafızasında bir adresi işaret etmek için kullanılacak.
...
- Terimlerin karşılıklarını bulurken ciddi sıkıntı yaşadığım için bir sözlük bölümü yapmaya karar verdim. [Terimler](/glossary)

- İşletim sistemi olarak Linux kullandım ama araçların çoğu diğer sistemlerde de mevcut.
